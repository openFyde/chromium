...
namespace {
  ...
  constexpr char kFydeDeviceIdBin[] = "/usr/bin/fyde_device_id";
...
void StatisticsCollector::ProceedToNextStep() {
  switch (step_) {
    case CollectStep::INITIALIZE:
      GetDeviceId();
      break;
    case CollectStep::GET_DEVICE_ID:
      GetLicenseId();
      break;
...
void StatisticsCollector::GetDeviceId() {
  step_ = CollectStep::GET_DEVICE_ID;;
  GetShellClient()->SyncExec(kFydeDeviceIdBin,
      base::BindOnce(&StatisticsCollector::OnGetDeviceId,
                     weak_factory_.GetWeakPtr()));
}

void StatisticsCollector::OnGetDeviceId(
...
void StatisticsCollector::GetLicenseTypeFromInfo(const std::string& license) {
  absl::optional<base::Value> json = base::JSONReader::Read(license, 0);
...
bool StatisticsCollector::GetUploadData(std::string* upload_data) {
  ...
  statistics.Set("fyde_device_id", statistics_.fyde_device_id);
  ...
  LOG(ERROR) << "==== FYDEOS TRACE ====[" << __func__ << "], jsonStr: " << jsonStr;
