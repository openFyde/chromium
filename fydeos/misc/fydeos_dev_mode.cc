...
void OnSetCommandFinished(const bool enabled, base::OnceCallback<void(bool)> callback, absl::optional<ShellState> state) {
...
void OnGetCommandFinished(base::OnceCallback<void(bool)> callback, absl::optional<ShellState> state) {
  if (!state || state->code != 0) {
