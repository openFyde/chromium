...
const char kFydeOSAccelerometerInitializeTimeoutInSeconds[] = "fydeos-accel-initialize-timeout";
...
int GetFydeOSAccelerometerInitializeTimeoutInSeconds() {
  std::string str = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(kFydeOSAccelerometerInitializeTimeoutInSeconds);
  if (str.empty())
    return 0;
  return std::stoi(str);
}
