...
const char kFydeBasicLicenseToken[] = "fyde-basic-license-token";
}
...
bool IsBasicLicenseDevice() {
  const std::string token = GetFydeBasicLicenseToken();
  return !token.empty();
}

std::string GetFydeBasicLicenseToken() {
  base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
  if (command_line->HasSwitch(kFydeBasicLicenseToken)) {
    return command_line->GetSwitchValueASCII(kFydeBasicLicenseToken);
  }
  return std::string();
}

std::string GetFydeAccountUrl() {
  base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
  if (command_line->HasSwitch(fydeos::switches::kFydeOSGaiaUrl)) {
    return command_line->GetSwitchValueASCII(fydeos::switches::kFydeOSGaiaUrl);
  } else {
    return fydeos::constants::kDefaultFydeOSGaiaUrl;
  }
}
