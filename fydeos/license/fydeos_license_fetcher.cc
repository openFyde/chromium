...
namespace {
  ...
  GURL GetFullURL(const std::string& id,
                  const std::string& fyde_device_id,
    ...
    url = net::AppendQueryParameter(url, "fyde_device_id", fyde_device_id);
...
void LicenseOnlineFetcher::StartFetch(
  ...
  */
  ...
  resource_request->url = GetFullURL(id, fyde_device_id, serial_number, is_new_license, oem_token, basic_token);
