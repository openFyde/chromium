...
void LicenseManager::IGetFydeDeviceId() {
  GetShellClient()->SyncExec(kFydeDeviceIdBin,
      base::BindOnce(&LicenseManager::OnGotFydeDeviceId,
                     weak_ptr_factory_.GetWeakPtr()));
}

void LicenseManager::OnGotFydeDeviceId(std::optional<fydeos::ash::ShellState> state) {
  if (!state || state->code != 0) {
    LOG(ERROR) << "Can't get license id.\n";
  } else {
    VLOG(1) << "Get fyde device id:" << state->result;
    fyde_device_id_ = state->result;
    fyde_device_id_.erase(std::remove(fyde_device_id_.begin(), fyde_device_id_.end(), '\n'), fyde_device_id_.end());
  }
}
