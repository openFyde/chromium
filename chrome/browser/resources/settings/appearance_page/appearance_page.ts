...
export class SettingsAppearancePageElement extends
  ...
  private openThemeUrl_() {
    ...
    const fydeosStoreBaseUrl = loadTimeData.getString('fydeosStoreBaseUrl');
    const themeHomeUrl_ = `${fydeosStoreBaseUrl}/?init=themes`;
  ...
  private themeChanged_(themeId: string) {
    ...
    if (themeId.length > 0 && themeId !== AUTOGENERATED_THEME_ID &&
      ...
      //---***FYDEOS BEGIN***---
      const fydeosStoreBaseUrl = loadTimeData.getString('fydeosStoreBaseUrl');
      this.themeUrl_ = loadTimeData.getBoolean('isFydeProfile') ?
        `${fydeosStoreBaseUrl}/?init=themes&appid=${themeId}` :
  ...
  private showThemesURL_(visibility: boolean, storeAppExists: boolean): boolean {
    return (visibility !== false) && (!loadTimeData.getBoolean('isFydeProfile') || storeAppExists);
  }
