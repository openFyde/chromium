...
void SessionLogHandler::GetFydeOsSystemInfo() {
  system_logs::SystemLogsFetcher* fetcher = system_logs::BuildAboutSystemLogsFetcher();
  fetcher->Fetch(base::BindOnce(&SessionLogHandler::OnFydeOSSystemInfoReceived, weak_ptr_));
}

void SessionLogHandler::OnFydeOSSystemInfoReceived(std::unique_ptr<system_logs::SystemLogsResponse> sys_info) {
  fydeos_system_info_ = "";
  ...
  if (ash::features::IsLogControllerForDiagnosticsAppEnabled()) {
    task_runner_->PostTaskAndReplyWithResult(
        ...
        base::BindOnce(
            ...
            // SessionLogHandler.
            base::Unretained(DiagnosticsLogController::Get()), path, fydeos_system_info_),
